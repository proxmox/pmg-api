...
sub reinject_local_mail {
    ...
    if ($dkim_sign) {
        ...
        eval { $entity = PMG::DKIMSign::sign_entity($entity, $dkim, $sender); };
        if ($@) {
...
sub reinject_mail {
    ...
    eval {
        ...
        if ($smtp->dataend()) {
            ...
            if (!$resid) {
                die sprintf(
                    "unexpected SMTP result - got: %s %s : WARNING\n",
                    $smtp->code, $resmess,
                );
            }
        } else {
    ...
    eval {
        ...
        my $parser = sub {
            ...
            if ($line =~ /^v\d$/) {
                ...
                die
                    "api version mismatch - expected $customcheck_apiver, got $customcheck_output_apiver !\n"
...
sub run_silent_cmd {
    ...
    eval { PVE::Tools::run_command($cmd, outfunc => $record_output, errfunc => $record_output); };
    my $err = $@;
...
our $db_service_list = ['pmgpolicy', 'pmgmirror', 'pmgtunnel', 'pmg-smtp-filter'];
...
sub update_node_status_rrd {
    ...
    my $data =
        "$ctime:$avg1:$maxcpu:$stat->{cpu}:$stat->{wait}:"
...
sub load_sa_descriptions {
    ...
    my @dirs = ('/usr/share/spamassassin', '/usr/share/spamassassin-extra');
...
sub get_existing_object_id {
    ...
    my $sth =
        $dbh->prepare("SELECT id FROM Object WHERE "
            . "Objectgroup_ID = ? AND "
            . "ObjectType = ? AND "
            . "Value = ?");

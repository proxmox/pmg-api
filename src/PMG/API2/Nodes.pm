...
__PACKAGE__->register_method({
    ...
    code => sub {
        ...
        return PMG::Utils::create_rrd_data("pmg-node-v1.rrd", $param->{timeframe},
            $param->{cf});
    },
});
...
__PACKAGE__->register_method({
    ...
    code => sub {
        ...
        my ($count, $lines) = PVE::Tools::dump_journal(
            $param->{start}, $param->{limit}, $param->{since}, $param->{'until'}, $service,
        );
...
__PACKAGE__->register_method({
    ...
    code => sub {
        ...
        $res->{pmgversion} = PMG::pmgcfg::package() . "/" . PMG::pmgcfg::version_text();

...
my $old_postgres_release = '15';
my $new_postgres_release = '17';
...
my ($min_pmg_major, $min_pmg_minor, $min_pmg_pkgrel) = (8, 2, 0);
...
sub check_pmg_packages {
    ...
    if (defined(my $pmg = $get_pkg->($pkg))) {
        ...
        # FIXME: better differentiate between 6.2 from bookworm or trixie
        ...
        my $kernel_version_is_expected = sub {
            ...
            return $version =~ m/^(?:6\.(?:8|14))/ if !$upgraded;

            if ($version =~ m/^6\.(?:8|1[14])\.\d[^~]*$/) {
                return 1;
...
sub check_running_postgres {
    ...
    if ($upgraded) {
        if ($version ne $new_postgres_release) {
            if ($version eq $old_postgres_release) {
                log_warn(
                    "Running postgres version is still $old_postgres_release. Please upgrade the database."
                );
            } else {
                log_fail(
                    "Running postgres version '$version' is not expected, was a previous upgrade left unfinished?"
                );
            }
        } else {
...
sub check_services_disabled {
    my ($upgraded_db) = @_;
    my $unit_inactive =
        sub { return $get_systemd_unit_state->($_[0], 1) eq 'inactive' ? $_[0] : undef };
    ...
    if (!$upgraded) {
        if (scalar($active_list->@*) < 1) {
            ...
            my $msg =
                "Not upgraded but core services still active. Consider stopping and masking them for the upgrade: \n    ";
            $msg .= join("\n    ", $active_list->@*);
            log_warn($msg);
        }
    } else {
        if (scalar($inactive_list->@*) < 1) {
            ...
            my $msg =
                "Already upgraded DB, but not all services active again. Consider unmasking and starting them: \n    ";
            $msg .= join("\n    ", $inactive_list->@*);
            log_warn($msg);
        } else {
            log_skip(
...
sub check_misc {
    ...
    PVE::Tools::dir_glob_foreach(
        ...
        sub {
            ...
            my $override =
                PVE::Tools::file_get_contents("$template_dir/$filename", 1024 * 1024);
    ...
    if (scalar(@override_but_unmodified)) {
        my $msg =
            "Found overrides in '/etc/pmg/templates/' for template, but without modification."

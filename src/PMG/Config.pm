...
EODESC
        ...
        dkim_selector => {
            ...
            type => 'string',
            format => 'dns-name', #see RFC6376 3.1
        },
        'dkim-use-domain' => {
            description =>
...
PVE::JSONSchema::register_format('transport-domain', \&pmg_verify_transport_domain);
...
PVE::JSONSchema::register_format('dnsbl-entry', \&pmg_verify_dnsbl_entry);
...
sub read_pmg_domains {
    ...
    if (defined($fh)) {
        while (defined(my $line = <$fh>)) {
            ...
            if ($line =~ m/^(\S+)\s.*$/) {
                my $domain = $1;
                $domains->{$domain} = { domain => $domain, comment => $comment };
                $comment = '';
            } else {
...
PVE::JSONSchema::register_format('tls-policy', \&pmg_verify_tls_policy);
...
PVE::JSONSchema::register_format('tls-policy-strict', \&pmg_verify_tls_policy_strict);
...
sub write_tls_policy {
    ...
    foreach my $destination (sort keys %$tls_policy) {
        ...
        PVE::Tools::safe_print($filename, $fh, "$entry->{destination} $entry->{policy}\n");
    }
}
...
PVE::JSONSchema::register_format('transport-address', \&pmg_verify_transport_address);
...
sub write_transport_map {
    ...
    foreach my $domain (sort keys %$tmap) {
        ...
        PVE::Tools::safe_print(
            $filename,
            $fh,
            "$data->{domain} $data->{protocol}:$host:$data->{port}\n",
        );
    }
}
...
sub get_template_vars {
    ...
    my $usepolicy = 0;
    $usepolicy = 1
        if $self->get('mail', 'greylist')
        || $self->get('mail', 'greylist6')
        || $self->get('mail', 'spf');
...
sub rewrite_config_spam {
    ...
    $changes = 1 if $self->rewrite_config_file('local.cf.in', '/etc/mail/spamassassin/local.cf');

    $changes = 1 if $self->rewrite_config_file('init.pre.in', '/etc/mail/spamassassin/init.pre');

    $changes = 1 if $self->rewrite_config_file('v310.pre.in', '/etc/mail/spamassassin/v310.pre');

    $changes = 1 if $self->rewrite_config_file('v320.pre.in', '/etc/mail/spamassassin/v320.pre');

    $changes = 1 if $self->rewrite_config_file('v342.pre.in', '/etc/mail/spamassassin/v342.pre');
...
sub rewrite_config_clam {
    ...
    return $self->rewrite_config_file('clamd.conf.in', '/etc/clamav/clamd.conf');
}

sub rewrite_config_freshclam {
    my ($self) = @_;

    return $self->rewrite_config_file('freshclam.conf.in', '/etc/clamav/freshclam.conf');
}
...
sub rewrite_config_postgres {
    ...
    $changes = 1 if $self->rewrite_config_file('pg_hba.conf.in', "$pgconfdir/pg_hba.conf");

    $changes = 1 if $self->rewrite_config_file('postgresql.conf.in', "$pgconfdir/postgresql.conf");
...
sub rewrite_config_postfix {
    ...
    if ($self->get('mail', 'tls')) {
        eval { PMG::Utils::gen_proxmox_tls_cert(); };
        syslog('info', "generating certificate failed: $@") if $@;
    }

    $changes = 1 if $self->rewrite_config_file('main.cf.in', '/etc/postfix/main.cf');

    $changes = 1 if $self->rewrite_config_file('master.cf.in', '/etc/postfix/master.cf');
...
sub rewrite_config {
    ...
    if (
        ($self->rewrite_config_postfix($rulecache) && $restart_services)
    ...
    if (
        ($self->rewrite_config_spam() && $restart_services)
        || $force_restart->{spam}
    ) {
    ...
    if (
        ($self->rewrite_config_clam() && $restart_services)
        || $force_restart->{clam}
    ) {
    ...
    if (
        ($self->rewrite_config_freshclam() && $restart_services)
    ...
    if (
        ($self->compare_smtp_filter_config() && $restart_services)
        || $force_restart->{spam}
    ) {

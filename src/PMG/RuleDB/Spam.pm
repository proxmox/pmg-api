...
sub save {
    ...
    if (defined($self->{id})) {
        ...
        $ruledb->{dbh}
            ->do("UPDATE Object SET Value = ? WHERE ID = ?", undef, $self->{level}, $self->{id});
...
sub get_blackwhite {
    ...
    eval {
        my $query =
            "SELECT * FROM UserPrefs WHERE " . "($cond) AND (Name = 'BL' OR Name = 'WL')";
    ...
    if (defined($queue->{sa_score}) && $queue->{sa_score} >= $self->{level}) {
        ...
        foreach my $t (@{ $msginfo->{targets} }) {
            ...
            if (
                ...
                syslog(
                    'info',
                    "%s: sender in user (%s) welcomelist",
                    $queue->{logid},
                    encode('UTF-8', $pmail),
                );
            } else {
...
sub analyze_spam {
    ...
    if ($msginfo->{testmode} || ($sa_score < 5)) {
        ...
        eval {
            ...
            $bayes =
                defined($status->{bayes_score})
                ? sprintf('%0.2f', $status->{bayes_score})
                : "undefined";
    ...
    syslog(
        'info',
        "%s: SA score=%s/%s time=%0.3f bayes=%s autolearn=%s hits=%s",
        $queue->{logid},
        $sa_score,
        $sa_max,
        $queue->{ptime_spam} / 1000.0,
        $bayes,
        $autolearn,
        $loglist,
    );
